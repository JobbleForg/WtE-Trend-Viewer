...
MAX_CHARTS = 8
INITIAL_VISIBLE = 4
_LAST_SESSION_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 "_last_session")
...
def _persist_upload(src_path, filename):
    """Copy the uploaded file to _last_session/ for autoload on next startup."""
    os.makedirs(_LAST_SESSION_DIR, exist_ok=True)
    dst = os.path.join(_LAST_SESSION_DIR, filename)
    try:
...
def on_file_upload(contents, filename, old_temp_path):
    ...
    _persist_upload(temp_path, filename)
...
def _build_figure(df_slice, selected_tags, tag_map, x_revision=None,
    ...
    # --- Step 4d: Add horizontal ruler line ---------------------------------
...
for _ci in range(1, MAX_CHARTS + 1):
    ...
    def update_chart(*args, _cid=_ci):
        ...
        goto_date, goto_time, win_min, win_hr, step, n_left, n_right, nn_data, show_limits_val, cursor_ts_val, ruler_y_val, ruler_time_str, start_time_iso, session_id = rest
...
# ---------------------------------------------------------------------------
# Autoload callback — restore last session on page load
# ---------------------------------------------------------------------------
...
def autoload_last_session(n_intervals, config):
    """Restore the last session (file, sheet, chart config) on page load."""
    ...
    # --- Load data (same as on_sheet_selected) ---
    try:
    ...
    # Build sheet-select options from the persisted file
    try:
    ...
    # --- Restore chart config (same as load_setup) ---
    ...
    results = [
        ...
        stats,           # data-stats
        pkg_options,     # pkg-select options
        file_path,       # temp-file-path (use persistent path)
        file_name,       # file-name-display
        sheet_options,   # sheet-select options
    ...
    for c in range(1, MAX_CHARTS + 1):
        ...
        # Series dropdown options + values
        series_vals = chart_cfg.get("series", [None] * NUM_SERIES)
        while len(series_vals) < NUM_SERIES:
            series_vals.append(None)
        for s_val in series_vals[:NUM_SERIES]:
            results.append(tag_options)   # options
            results.append(s_val)         # value
...
# ---------------------------------------------------------------------------
# Auto-save callback — periodically snapshot session state for autoload
# ---------------------------------------------------------------------------
...
def autosave_session(n_intervals, session_id, file_name, sheet_name, visible, sync_state, *chart_args):
    ...
    persistent_path = os.path.join(_LAST_SESSION_DIR, file_name) if file_name else None
